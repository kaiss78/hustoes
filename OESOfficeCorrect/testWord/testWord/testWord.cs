    ...
    public partial class testWord : UserControl
    {
        ...
        void generateTree()
        {
            ...
            foreach (Word.Paragraph p in doc.Paragraphs)
            {
                ...
                Push(tmpNode);
                AddNode(new ItemObject("对齐与缩进", p, WordType.Indent));
                AddNode(new ItemObject("文字属性", p.Range, WordType.Font));
                AddNode(new ItemObject("首字下沉", p.DropCap, WordType.DropCap));
                AddNode(new ItemObject("分栏", p.Range.PageSetup.TextColumns, WordType.TextColumns));
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (selectedNode.Parent == null)
                return;
            if ((selectedNode.Tag as ItemObject).type == WordType.Null)
                return;
            ...
            while ((tmpNode.Tag as ItemObject).type != WordType.Null)       //获取路径上的节点
            {
