...
namespace OES.UPanel
{
    public partial class BulkImport : UserPanel
    {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    switch (list[i][3].ToLower())
                    {
                        ...
                        case "word":
                            ...
                            File.Move(basePath + list[i][4], InfoControl.config["WordPath"] + "p" + ids[i] + ".doc");//配置word的路径
        ...
        {                                                         //ids是每道题的题号,basePath为父目录的路径
                ...
                {
                    case "Comp":
                        File.Move(basePath + list[5], InfoControl.config["CompletionPath"] + "p" + ids + ".cpp");
                        InfoControl.ClientObj.SaveCppCompletion(ids, InfoControl.User.Id);
                        break;
                    case "Modi":
                        File.Move(basePath + list[5], InfoControl.config["ModificationPath"] + "p" + ids + ".cpp");
                        InfoControl.ClientObj.SaveCppModification(ids, InfoControl.User.Id);
                        break;
                    case "Prog":
                        File.Move(basePath + list[5], InfoControl.config["FunctionPath"] + "p" + ids + ".cpp");
                        File.Move(basePath + list[6], InfoControl.config["FunctionPath"] + "a" + ids + ".cpp");
